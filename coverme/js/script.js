...
function CoverCtrl($scope, $location) {
  ...
  {'name':'NovaSeq S4', 'id':'novas4', 'reads':8000000000},
  {'name':'NextSeq 2000 P2', 'id':'ns2p2', 'reads':400000000},
  {'name':'NextSeq 2000 P3', 'id':'ns2p3', 'reads':1200000000},
  {'name':'NextSeq 500 High Output', 'id':'nshi', 'reads':400000000},
  {'name':'NextSeq 500 Mid Output', 'id':'nsmid', 'reads':130000000},
  ...
  $scope.coverage_string = function() {
    ...
    if(isFinite(coverage)) {
		 if(readspersample > 999)
		 {
			coverage = toFixed(coverage, 1) + "x coverage" + " (" + toFixed(readspersample)/1000 + " billion reads per sample)";
		 }
		 else
		 {
			coverage = toFixed(coverage, 1) + "x coverage" + " (" + toFixed(readspersample) + " million reads per sample)";
		 }
    }
    else {
      coverage = '-.-';
